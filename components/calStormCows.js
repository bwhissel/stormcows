...
calStormCows.prototype = {
  ...
  set pendingApiRequest(aValue) {
		...
		// When we turn off the pendingApiRequest flag, process
		// anything in the pendingApiRequestListeners queue.
		if (aValue == false) {
  ...
  getCachedItems:	function cSC_getCachedItems(aItemFilter, aCount,
  ...
  refresh: function cSC_refresh() {
		...
		// Setting the last sync to null forces the next getItems call
		// to make an API request rather than returning a cached result.
		this.mLastSync = null;
  ...
  adoptItem: function cSC_adoptItem(aItem, aListener) {
	    ...
	    if (this.itemType == 'events' && !isEvent) {
				throw new Components.Exception('This calendar only accepts events.',
																			 Components.results.NS_ERROR_UNEXPECTED);
	    }
	    if (this.itemType == 'todos' && isEvent) {
				throw new Components.Exception('This calendar only accepts todos.',
																			 Components.results.NS_ERROR_UNEXPECTED);
	    }
  ...
  modifyItem: function cSC_modifyItem(aNewItem, aOldItem, aListener) {
	    ...
	    this.notifyOperationComplete(aListener, e.result,
  ...
  modifyItem_callback: function cSC_modifyItem_callback(aStatus, aNewItem, aOldItem, aListener) {
	    ...
	    stormcowsLogger.debug('calStormCows.js:modifyItem_callback()',
														'Got an error from the API request');
	    this.notifyOperationComplete(aListener, Components.results.NS_ERROR_UNEXPECTED,
																	 Components.interfaces.calIOperationListener.MODIFY,
																	 null, 'Unable to modify task.');
		}
  },
  ...
  getItems_callback: function cSC_getItems_callback(aStatus, aItems, aListener) {
		...
		if (aStatus == -1) {
	    aListener.onGetResult(this.superCalendar,	Components.results.NS_OK,
														calItemType, null, aItems.length,	aItems);
	    this.notifyOperationComplete(aListener, Components.results.NS_OK,
																	 Components.interfaces.calIOperationListener.GET,
																	 null, null);
		} else {
	    if (aStatus == rtmClient.results.RTM_API_OK) {
				stormcowsLogger.debug('calStormCows.js:getItems_callback()',
															'Refreshing the task cache');
				this.mTaskCache[this.id] = {};
